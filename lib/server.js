...
function Server(serverUrl, serverPort, database) {
   ...
   */
  app.get("/api/1/states.json", function (req, res) {
    database.getStates(function (err, states) {
      if (err) {
        res.status(400).send({
          error: err
        })
  ...
  app.get("/api/1/clear.json", function (req, res) {
    ...
    if (pricers) {
      database.clearPricers(function (err, cleared) {
        if (err) {
          res.send(err)
        } else {
          res.send(cleared)
    ...
    if (monitor) {
      monitoring.clear(function (err, cleared) {
        if (err) {
          res.send(err)
        } else {
          res.send(cleared)
